...
export default function BarcodSettingsBox({
  ...
  const toggleToDeleteItem = () => {
    return isEnabled ? (
      ...
      <Text style={[styles.deleteItemText]}>Toggle to delete</Text>
    );
  };

  const toggleToAddItem = () => {
    return isEnabled1 ? (
      <Text style={styles.addItemText}>Toggle to use product</Text>
    ) : (
      <Text style={[styles.addItemText]}>Toggle to scan new item</Text>
    );
  };
...
// THIS IS THE OLD CODE FOR THE BARCODE SETTINGS BOX
